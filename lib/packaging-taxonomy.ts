...
export interface PackagingDriversDetail {
  ...
  // 0 = a pure guess, 10 = certain, in how far `trendRelevance` can be trusted.
  // Trend judgement rests on the model's own memory of what was current, which
  // is frozen at its training cutoff and blind to when the video actually ran,
  // so this is expected to sit low until real grounding data (upload date, a
  // trends feed) is fed in to raise it. Read `trendRelevance` through this: a
  // high score with low confidence is a hunch, not a finding.
  trendRelevanceConfidence: number
}
  ...
  properties: {
    ...
    drivers: {
      ...
      properties: {
        ...
        trendRelevanceConfidence: ordinalSchema,
      },
    },
  },
} as const
...
function isModelDriversDetail(value: unknown): boolean {
  ...
  return (
    ...
    isOrdinal(t.trendRelevanceConfidence)
  )
}
...
export async function generatePackagingTaxonomy(
  ...
  const instructions = await resolvePrompt("packaging_taxonomy")
