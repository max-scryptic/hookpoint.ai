...
-- footage is in, pointing at what the upload just unlocked: the retention
-- chart's highlights, which now play that moment back from the file, and the
-- per-window tabs the deeper analysis adds alongside "Script". Each one is
-- retired the moment the creator uses the thing it points at — at that point
-- they know the feature is there and a second showing is just noise.
--
