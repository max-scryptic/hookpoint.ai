...
const COLUMNS =
  "id, user_id, analysed_video_id, youtube_video_id, original_filename, storage_provider, storage_path, file_size_bytes, mime_type, uploaded_duration_seconds, youtube_duration_seconds, duration_difference_seconds, duration_validation_status, filename_validation_status, filename_similarity_score, validation_status, upload_status, failure_reason, delete_after, created_at, updated_at"
...
export async function replaceSourceFile(
  supabase: SupabaseClient,
  input: CreateSourceFileInput,
): Promise<{ sourceFile: SourceFile; previousStoragePath: string | null }> {
  ...
  return { sourceFile, previousStoragePath: existing?.storagePath ?? null }
}
