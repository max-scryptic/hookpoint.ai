-- Account-wide log of every paid AI/media call the platform makes, so the
-- admin area can show exactly which calls we run and what they cost. Each row
-- is one call: the user it was run for, the kind of call (light pacing /
-- packaging analysis, the per-window deep-analysis vision/audio/synthesis
-- calls, and Qencode transcoding), the model/provider, token counts and the
-- derived dollar cost, and the moment it happened.
--
...
create table public.llm_calls (
  ...
  -- Who the call was run for. Kept (set null) when the account is deleted so
  -- accounting history survives; user_email preserves the human identity.
  user_id uuid references auth.users(id) on delete set null,
  user_email text,
  -- Optional link back to the video the call was part of, when there is one.
  analysed_video_id uuid,
  -- The kind of call. Kept in sync with LlmCallType in lib/llm-calls.ts.
  ...
  -- When the call was made. Explicit column (not just created_at) so the admin
  -- datetime-window filter reads naturally, though they are the same instant.
  created_at timestamptz not null default now()
);

-- Admin listing filters and orders by time, user and type.
