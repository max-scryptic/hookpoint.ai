...
export function hasLightAnalysisEvidence(
  ...
  return Boolean(evidence.pacing || evidence.packaging || evidence.retention)
}
...
export async function getLightAnalysisEvidence(
  ...
  const [pacing, packaging, retention] = await Promise.all([
  ...
  return { pacing, packaging, retention }
}
