...
// Something to try on the next upload, filed under the surface it changes.
// Every surface shows one tip and one only, so a recommendation is rendered
// only where that surface has no tip of its own to lead with (reports stored
// before schema version 2); the rest are kept with the report but not shown.
