...
const EVENT_SYNTHESIS_INSTRUCTIONS = [
  ...
  "Identify the distinct, timestamped moments within the window that genuinely and non-obviously explain the retention outcome — a hard cut, a topic change in the transcript, a shift in pacing or energy, on-screen text or a graphic appearing/disappearing, a freeze or dead air, and so on. In a hold, prefer evidence of what successfully maintained continuity, clarity, novelty, or energy. A single window can have more than one, but do not manufacture one per cut: most cuts and most frames are unremarkable.",
  "Judge editing and pacing as deviations from the given baseline, not in absolute terms: a static, low-cut, low-energy stretch only explains a drop when it is slower or flatter than this video's own norm, and a burst of cuts or rising energy only explains a gain when it is livelier than the norm. For a hold, either continuity near the norm or a well-timed supported change can be relevant, but only surface it when the other evidence makes its role non-obvious and specific. Where a metric drives an event, reference the deviation in the narrative (for example 'your cuts fall to about 2 per minute here versus roughly 11 across the video').",
