...
// underperformance. Gains use cumulative replay/return magnitude. Hooks are
// mandatory and handled separately by the selector.
...
// gain are reserved before remaining slots are filled by score.
export function selectDeepAnalysisWindows(
  ...
  const ranked = (kind: "drop_off" | "gain") =>
    eligible
  ...
  for (const candidate of [ranked("drop_off")[0], ranked("gain")[0]]) {
